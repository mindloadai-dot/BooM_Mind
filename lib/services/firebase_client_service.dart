...
class FirebaseClientService extends ChangeNotifier {
  ...
  Future<void> _initializeMessaging() async {
    try {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized) {
        ...
        debugPrint('✅ FCM Device Token obtained: ${_deviceToken?.substring(0, 20)}...');
  ...
  /// Sign in with Google (modernized)
  Future<AuthResult> signInWithGoogle() async {
    try {
      ...
      // Use Firebase provider path for mobile
  ...
  Future<AuthResult> signInWithApple() async {
    try {
      ...
      return AuthResult(success: false, error: 'Apple sign in failed: $e');
    }
  }
  ...
  Future<AuthResult> signInWithBiometrics() async {
    try {
      ...
      if (availableBiometrics.isEmpty) {
        return AuthResult(success: false, error: 'No biometric methods configured');
      }
      ...
      if (isAuthenticated) {
        ...
        if (_currentUser != null) {
          ...
          return AuthResult(success: false, error: 'No user account found. Please sign in first.');
        }
      } else {
        return AuthResult(success: false, error: 'Biometric authentication failed');
      }
    } catch (e) {
      return AuthResult(success: false, error: 'Biometric authentication error: $e');
    }
  }
  ...
  Future<bool> uploadStudySet(StudySet studySet, {
  ...
  Stream<List<StudySet>> getUserStudySets() {
    ...
    return _repository.getUserStudySets(currentUserId!)
        .map((firestoreStudySets) => firestoreStudySets
            .map((fs) => fs.toStudySet())
            .toList());
  }
  ...
  Future<String?> uploadFile(Uint8List fileBytes, String fileName, String userId) async {
    try {
  ...
  Future<String?> uploadPDF(File pdfFile, String userId) async {
    try {
      ...
      final fileName = '${DateTime.now().millisecondsSinceEpoch}_${pdfFile.path.split(RegExp(r'[/\\]')).last}';
  ...
  Future<String?> uploadImageAsPdf(Uint8List imageBytes, String userId) async {
    try {
      final pdfBytes = await DocumentProcessor.convertImageToPdfBytes(imageBytes);
  ...
  /// Save quiz result
  Future<bool> saveQuizResult(String studySetId, String quizId, String quizTitle,
      int score, int totalQuestions, Duration timeTaken, List<String> incorrectAnswers) async {
    try {
      if (!isAuthenticated) return false;
  ...
  Future<bool> updateNotificationPreferences(Map<String, dynamic> preferences) async {
    ...
    try {
      await _repository.updateNotificationPreferences(currentUserId!, preferences);
      return true;
    } catch (e) {
  ...
  Future<void> logAnalyticsEvent(String eventName, Map<String, Object?>? parameters) async {
    try {
      ...
      final Map<String, Object>? safeParameters = parameters?.cast<String, Object>();
  ...
  /// Log user engagement
  Future<void> logUserEngagement(String action, Map<String, dynamic>? metadata) async {
  ...
  Future<void> logSubscriptionEvent(String event, String plan, double? price) async {
  ...
  Future<void> setUserProperties({
    ...
    try {
      ...
      if (totalStudySets != null) {
        await _analytics.setUserProperty(name: 'total_study_sets', value: totalStudySets.toString());
      }
