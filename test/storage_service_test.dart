  ...
  group('Storage Service Tests', () {
    ...
    group('Storage Limits Tests', () {
      test('Enforces storage budget MB limit', () async {
        ...
        expect(stats['totalBytes'],
            lessThanOrEqualTo(StorageConfig.storageBudgetMB * 1024 * 1024));
      });
      ...
      test('Enforces max local sets limit', () async {
        ...
        final stats = storageService.getStorageStats();
        expect(
            stats['totalSets'], lessThanOrEqualTo(StorageConfig.maxLocalSets));
      });
      ...
      test('Enforces max local items limit', () async {
        ...
        // Check that we don't exceed the limit
        final stats = storageService.getStorageStats();
        expect(stats['totalItems'],
            lessThanOrEqualTo(StorageConfig.maxLocalItems));
      });
    });
    ...
    group('LRU Eviction Tests', () {
      ...
      test('Evicts stale sets (>120 days)', () async {
        ...
        final staleSet = StudySetMetadata(
          ...
          items: 200,
          lastOpenedAt: DateTime.now()
              .subtract(const Duration(days: 130)), // 130 days old
    ...
    group('Storage Warning Tests', () {
      test('Shows storage warning at 80% usage', () async {
        ...
        int setId = 1;
        final targetBytes = (StorageConfig.storageBudgetMB *
                StorageConfig.warnAtUsage *
                1024 *
                1024)
            .round();
        int currentBytes = 0;
      ...
      test('Does not show warning below 80% usage', () async {
        ...
        int setId = 1;
        final targetBytes =
            (StorageConfig.storageBudgetMB * 0.7 * 1024 * 1024).round();
        int currentBytes = 0;
    ...
    group('Low Free Space Tests', () {
      test('Reduces budget when free space < 1GB', () async {
        ...
        final normalBudget =
            StorageConfig.getCurrentBudgetMB(5000); // 5GB free space (in MB)
        final lowBudget =
            StorageConfig.getCurrentBudgetMB(500); // 0.5GB free space (in MB)
      ...
      test('Uses low mode budget when free space is low', () async {
        ...
        final stats = storageService.getStorageStats();
        expect(stats['totalBytes'],
            lessThanOrEqualTo(StorageConfig.lowModeBudgetMB * 1024 * 1024));
      });
    });
    ...
    group('Storage Stats Tests', () {
      ...
      test('Calculates usage percentage correctly', () async {
        ...
        final stats = storageService.getStorageStats();
        expect(stats['usagePercentage'],
            closeTo(0.5, 0.1)); // Within 10% tolerance
      });
    });
