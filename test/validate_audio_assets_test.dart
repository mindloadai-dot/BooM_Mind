...
void main() {
  group('Audio Assets Validation', () {
    test('manifest.json exists and is valid', () async {
      // Load manifest
      final manifestContent = await rootBundle.loadString('assets/audio/manifest.json');
      expect(manifestContent, isNotEmpty, reason: 'Manifest file should not be empty');
      
      // Parse JSON
      final manifestJson = jsonDecode(manifestContent);
      expect(manifestJson, isA<Map<String, dynamic>>(), reason: 'Manifest should be valid JSON object');
      
      // Validate required fields
      expect(manifestJson['tracks'], isA<List>(), reason: 'Manifest should contain tracks array');
      expect(manifestJson['defaultOrder'], isA<List>(), reason: 'Manifest should contain defaultOrder array');
      expect(manifestJson['metadata'], isA<Map>(), reason: 'Manifest should contain metadata object');
      
      // Validate metadata
      final metadata = manifestJson['metadata'];
      expect(metadata['version'], isNotNull, reason: 'Metadata should include version');
      expect(metadata['created'], isNotNull, reason: 'Metadata should include creation date');
      expect(metadata['crossfadeSupported'], isA<bool>(), reason: 'Metadata should specify crossfade support');
      expect(metadata['loopingSupported'], isA<bool>(), reason: 'Metadata should specify looping support');
    });
    ...
    test('all tracks in manifest are valid', () async {
      ...
      expect(tracks, isNotEmpty, reason: 'Manifest should contain at least one track');
      ...
      for (int i = 0; i < tracks.length; i++) {
        ...
        // Validate required fields
        expect(track['key'], isA<String>(), reason: '$trackName should have key field');
        expect(track['filename'], isA<String>(), reason: '$trackName should have filename field');
        expect(track['lengthSec'], isA<num>(), reason: '$trackName should have lengthSec field');
        expect(track['sampleRate'], isA<int>(), reason: '$trackName should have sampleRate field');
        expect(track['channels'], isA<int>(), reason: '$trackName should have channels field');
        expect(track['title'], isA<String>(), reason: '$trackName should have title field');
        expect(track['description'], isA<String>(), reason: '$trackName should have description field');
        
        // Validate field values
        expect(track['key'], isNotEmpty, reason: '$trackName key should not be empty');
        expect(track['filename'], isNotEmpty, reason: '$trackName filename should not be empty');
        expect(track['lengthSec'], greaterThan(0), reason: '$trackName length should be positive');
        expect(track['channels'], equals(2), reason: '$trackName must be stereo (2 channels) for binaural beats');
        expect(track['sampleRate'], anyOf([44100, 48000]), reason: '$trackName sample rate must be 44.1kHz or 48kHz');
        expect(track['title'], isNotEmpty, reason: '$trackName title should not be empty');
        expect(track['description'], isNotEmpty, reason: '$trackName description should not be empty');
      }
    });
    ...
    test('default order references valid tracks', () async {
      ...
      expect(defaultOrder, isNotEmpty, reason: 'Default order should not be empty');
      
      for (final key in defaultOrder) {
        expect(key, isA<String>(), reason: 'Default order should contain string keys');
    ...
    test('UltraAudioManifest can parse manifest', () async {
      ...
      expect(() => UltraAudioManifest.fromJson(manifestJson), returnsNormally,
        reason: 'UltraAudioManifest should parse valid manifest without errors');
      
      final manifest = UltraAudioManifest.fromJson(manifestJson);
      
      // Validate parsed manifest
      expect(manifest.tracks, isNotEmpty, reason: 'Parsed manifest should have tracks');
      expect(manifest.defaultOrder, isNotEmpty, reason: 'Parsed manifest should have default order');
      expect(manifest.version, isNotEmpty, reason: 'Parsed manifest should have version');
      expect(manifest.created, isA<DateTime>(), reason: 'Parsed manifest should have valid creation date');
    ...
    test('all required binaural frequencies are present', () async {
      ...
      for (final frequency in ['Alpha', 'Beta', 'Gamma', 'Theta']) {
        final hasFrequency = trackTitles.any((title) => 
          title.toLowerCase().contains(frequency.toLowerCase()));
    ...
    test('track lengths are reasonable for binaural sessions', () async {
      ...
      for (final track in tracks) {
        ...
        expect(lengthSec, greaterThanOrEqualTo(300), // 5 minutes
          reason: '$trackName should be at least 5 minutes long for effective binaural beats');
        expect(lengthSec, lessThanOrEqualTo(1200), // 20 minutes
          reason: '$trackName should be at most 20 minutes to avoid large file sizes');
      }
    });
  });
