...
exports.generateStudyMaterial = exports.generateQuiz = exports.generateFlashcards = exports.testOpenAI = void 0;
...
async function retryOpenAICall(apiCall, operationType, maxRetries = 3) {
    ...
    if (lastError.message?.includes('Overloaded') || lastError.status === 503 || lastError.status === 529) {
        throw new https_1.HttpsError('resource-exhausted', 'OpenAI service is currently overloaded. Please try again in a few moments.', {
 ...
 */
async function validateAppCheck(request) {
    try {
 ...
 */
exports.testOpenAI = (0, https_1.onCall)({
    region: 'us-central1',
    timeoutSeconds: 30,
    memory: '256MiB',
...
`;
        ...
        throw new https_1.HttpsError('internal', `OpenAI test failed: ${error.message}`, {
...
Key facts only.`;
        ...
        const response = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('OpenAI API timeout')), CONFIG.TIMEOUT_MS))
        ]);
...
4 options each.`;
        ...
        logger.error('OpenAI API call failed:', error);
        throw new https_1.HttpsError('internal', 'Failed to generate quiz questions');
    }
});
/**
