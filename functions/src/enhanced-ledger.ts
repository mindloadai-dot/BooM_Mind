 ...
 */
export const writeLedgerEntry = functions.https.onCall(async (data: any, context) => {
  ...
  try {
    ...
    await logTelemetry(admin.firestore(), 'ledger.write_error', {
      userId,
      action,
      tokens,
      requestId,
 ...
 */
async function isDuplicateRequest(db: admin.firestore.Firestore, requestId: string, userId: string): Promise<boolean> {
  try {
    const now = new Date();
 ...
 */
function updateAccountBalance(accountData: any, action: string, tokens: number): any {
  switch (action) {
    case 'credit':
      return {
        ...accountData,
 ...
 */
function debitFromAccount(accountData: any, tokens: number): any {
 ...
 */
async function updateAccountBalanceNonAtomic(db: admin.firestore.Firestore, userId: string, action: string, tokens: number, entryId: string): Promise<void> {
  const accountRef = db.collection(ACCOUNTS_COLLECTION).doc(userId);
 ...
 */
export const getLedgerEntries = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    if (since) {
      ...
      query = query.where('timestamp', '>', admin.firestore.Timestamp.fromDate(sinceDate));
    }
 ...
 */
export const getUserTokenAccount = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const reconcileUserLedger = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
async function performUserReconciliation(userId: string): Promise<any> {
  try {
    const db = admin.firestore();
 ...
 */
export const getLedgerStats = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const cleanupOldLedgerEntries = functions.https.onCall(async (data: any, context) => {
