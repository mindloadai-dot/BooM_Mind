...
const PRODUCT_TOKEN_MAP: { [key: string]: number } = {
 ...
 */
export const verifyLogicPackPurchase = functions.https.onCall(async (data: any, context) => {
  ...
  try {
    ...
    await logTelemetry(admin.firestore(), 'purchase.verification_error', {
      purchaseId: transactionId,
      userId,
      productId,
 ...
 */
async function checkPurchaseReplay(db: admin.firestore.Firestore, transactionId: string, userId: string): Promise<any | null> {
  try {
 ...
 */
async function checkPurchaseCache(db: admin.firestore.Firestore, transactionId: string, userId: string): Promise<any | null> {
  try {
 ...
 */
async function cachePurchaseResult(db: admin.firestore.Firestore, transactionId: string, userId: string, result: any): Promise<void> {
  try {
    await db
        .collection('purchase_cache')
        .doc(`${userId}_${transactionId}`)
 ...
 */
async function storePurchaseReceipt(db: admin.firestore.Firestore, userId: string, transactionId: string, productId: string, tokens: number, platform: string, purchaseToken: string): Promise<void> {
  try {
    await db
 ...
 */
async function verifyApplePurchase(receiptData: any, productId: string): Promise<{ isVerified: boolean; tokens: number; errorMessage?: string }> {
  try {
 ...
 */
async function verifyGooglePurchase(receiptData: any, productId: string): Promise<{ isVerified: boolean; tokens: number; errorMessage?: string }> {
  try {
 ...
 */
async function writeLedgerEntry(db: admin.firestore.Firestore, userId: string, action: string, tokens: number, requestId: string, source: string, metadata: any): Promise<{ success: boolean; error?: string }> {
  try {
 ...
 */
export const restoreLogicPackPurchases = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const getPurchaseHistory = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const getPurchaseVerificationStatus = functions.https.onCall(async (data: any, context) => {
  // Validate authentication
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const clearPurchaseCache = functions.https.onCall(async (data: any, context) => {
 ...
 */
async function logTelemetry(db: admin.firestore.Firestore, event: string, parameters: any): Promise<void> {
  try {
